...
fn process_restart_asset0(
    ...
    let current_market_id = percolator_accounting::read_asset_market_id(
        &market_slab.try_borrow_data()?,
        0,
    )
    .map_err(|_| ProgramError::InvalidAccountData)?;
...
fn process_replace_frozen_holding(
    ...
    let config = Config::deserialize(&config_account.try_borrow_data()?)?;
    let book = load_book_header(&book_account.try_borrow_data()?)?;
    // Preserve every custody separation enforced by book initialization and
    // reconfiguration. A savings alias would make execute reject permanently;
    // a same-mint COIN-sink alias would recycle bought rewards as auction budget.
    if book.config != *config_account.key
        || book.state != BOOK_STATE_OPEN
        ...
        || *replacement_holding.key == book.coin_sink
    {
        return Err(ProgramError::InvalidAccountData);
    }
