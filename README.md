...
| `twap-program/` | Post-genesis surplus auction and constrained asset-0 custodian. It can pull insurance above the protected floor only while asset 0 has no open position or unresolved loss state, run repeating uniform-price buybacks, cumulatively split bought COIN between burn and a bound reward vault, and make auction placements and inbound insurance donations one-shot through book-local bidder and per-donor nonces. Delayed auction-share, reserve, COIN-sink, and four-way-economics actions bind exact current state and consume config-bound policy sequences, so even a no-op correction invalidates an older approved action. Auction share and four-way economics share one sequence because they mutate the same split; reserve and COIN sink each use an independent sequence. These nonce PDAs only store a sequence: they cannot sign, hold tokens, name a recipient, grant authority, or move user principal. Pool-bound recovery stays bound to the original genesis pool. TWAP also accepts that exact pool's cross-backing earnings transit only when its entire balance is transferred to a clean token account owned by the bound Squads vault. After resolution, terminal protocol insurance follows the same fixed governance-owner rule; a pool-less config can sign only the controller's amountless, provider-bound external-backing return. |
...
`557f15b63c7a5f9e058a2c384ca8728829dc2b63` and its engine layout dependency to
`e3ea7251ed75f0111ba3cf5a4b87961f6c7cca63`.
...
has a caller fee floor but no user maximum, so an increase could reprice an already-signed trade.
Delayed ordinary-fee and fee-redirect updates share one exact controller-bound fee-economics
sequence, so executing a correction invalidates every older approved update in either class. Safe
...
free slot or the exact weakest bidder and bid legs it may evict. Each successful placement also consumes
the bidder's monotonic book-local nonce, so an independently signed retry cannot become admissible again
...
policy remains bidirectional, but its independent consumed sequence means an older approved action
cannot overwrite either a changed-state or no-op correction.
...
6. **Kickstart.** Futarchy can use retained surplus to initialize insurance/backing for approved
   ...
   in-place restart into a replacement market generation. Each donation also consumes the exact
   donor's monotonic nonce, so only one independently signed retry for that intent can execute.
...
savings, and buyback shares plus the savings account. Auction-share and four-way-economics updates
consume one shared `(config, policy)` sequence because either setter changes the same split. Reserve and
COIN-sink updates each consume a separate sequence. Every successful action, including a no-op
reaffirmation, therefore invalidates older approvals in its complete policy family.
Anyone can pre-fund the deterministic address or top up the Squads vault's one-time rent; robust PDA
allocation tolerates that pre-funding. The PDA has no signer, token, recipient, or close path.
...
Hybrid, EWMA, authenticated-mark, and oracle-restart actions also append the expected value of one
shared oracle-policy sequence and pass its canonical controller-bound account after the generation
witness. The sequence serializes delayed oracle mutations across every asset: executing a correction,
including a no-op correction, permanently invalidates older approved oracle actions. The controller
strips both controller-only accounts before CPI. The sequence account cannot sign, hold tokens, select
a recipient, grant authority, or move collateral.
