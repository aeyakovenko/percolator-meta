...
- **Lifecycle is separate from custody.** The immutable `market-controller` PDA holds
  ...
  authority and operator must also be one identity even while empty, because later public trades
  accrue withdrawable fee insurance without using the controller donation path. Donation
...
fully refunded nominal allocations to lower executable bids in the same round; ordinary integer
rounding remains in holding for a later round. A bid that cannot transact a whole-atom pair cannot
reserve nominal budget or set the marginal price. If every eligible bid is integer-infeasible, the
aged book settles for permissionless refunds without spending collateral. Insurance deposited for
other assets is never counted toward that surplus. Bought COIN can be split between burn and a
canonical dynamic reward vault. The collateral savings reserve must be distinct from the auction
holding; an aliased configuration fails atomically before either surplus pull, so savings cannot
enlarge the buy budget.
...
restore the controller operator, so a later oracle restart does not inherit the one-shot role. A
healthy canonical controller account retains the complete balance. If that account is permanently
frozen, an empty replacement may instead forward the exact amount to a clean governance-owned
account and close atomically; a caller cannot redirect or fragment it. While live, no return can
...
The insurance authority and operator must also match before handoff: an unrelated operator could
otherwise skim fee insurance generated by later public trades despite an initially empty balance.
