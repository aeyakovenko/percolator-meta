...
| `twap-program/` | Post-genesis surplus auction and constrained asset-0 custodian. It can pull only insurance above the monotonic floor, run repeating uniform-price buybacks, burn or route bought COIN to a bound reward vault, accept inbound insurance donations, apply the trade-fee setter and clear predecessor backing-fee policies, and timelock-restart only an empty recovering asset 0. If an external collateral issuer freezes the shared settlement account or canonical auction holding while the book is open, anyone may bind a clean empty replacement owned by the same PDA. Settled bidder payouts can never be abandoned or redirected; replacing holding abandons only its already-frozen protocol surplus, while reserved insurance remains in Percolator. Pool-bound recovery stays bound to the original genesis pool; a signing owner may use its fixed live full-exit proxy, whose value accounts are all revalidated by Subledger. After resolution, either that pool proves zero claims or a current-layout pool-less config proves it began empty; TWAP can then route the terminal protocol floor through an empty TWAP-owned transit only to a clean Squads-vault-owned account. A pool-less config can also sign only the controller's amountless, provider-bound asset-0 backing return. |
...
retains a permissionless, hardcoded zero-only migration path. Backing fees can be re-enabled only after batch-safe accounting
...
3. **Bootstrap vote.** The default bootstrap delay is six 30-day months and is configurable in the
   ...
   votable until its declared entry capacity is full and its allocations total exactly the fixed
   supply, so no creator action remains after voting and no Genesis COIN is silently omitted.
...
Predecessor fixed-distribution configs enforce the same immutable end: registration closes before
that slot, and their finalize window cannot admit new LP, funding, trader, or capital point growth.
