...
4. **Seal 100% of supply.** After the deadline, anyone can trigger a qualifying proposal. The
   ...
   A strict tie remains an unsealed COIN-distribution outcome. Once fallback refunds open, the first
   return permanently closes triggering before any vote or quorum denominator changes, so a cranker
   cannot select one tied side by refunding the other. The tie cannot strand collateral or slab
