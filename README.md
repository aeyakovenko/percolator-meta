...
- **Custody transitions are fixed.** Asset-0 custody moves
  ...
  floor, and every approved raise or no-op correction consumes the floor's config-bound policy
  sequence, so an older irreversible pause cannot land afterward. A fixed re-handoff may replace
  only its recorded pool-principal component. Recovery can
