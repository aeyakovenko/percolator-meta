...
| `distribution/` | Holds the fixed COIN pool in a vault. A proposal is one incrementally grown on-chain account of up to ~10k `(pubkey, amount)` entries; the sealed winner's recipients **claim** permissionlessly, **unclaimed is burned**. Never mints. The `authority` (whichever decider PDA) and `claim_window_slots` are bound into the config seed, making it decider-pluggable without leaving the claim/burn deadline first-writable. |
