...
// An authenticated target must enter engine risk checks atomically. Otherwise an attacker can move
// an adverse short from a funded portfolio into a thin sacrificial portfolio at the stale target,
// withdraw the funded account, and externalize the later loss onto an independent long.
#[test]
fn pending_authmark_cannot_be_migrated_to_thin_portfolio() {
    ...
    let run = |migrate_after_mark: bool| {
        ...
        .expect("authenticate adverse target before migration");
