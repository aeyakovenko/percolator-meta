...
// A backing provider may exit only against already-settled market state. Once an independent
// authenticated oracle commits an adverse mark, the provider must remain at risk until bounded
// public cranks realize the matching winner claim.
#[test]
fn committed_mark_cannot_release_backing_before_public_accrual() {
    const ENTRY_PRICE: u64 = 1_000_000;
    const EXIT_PRICE: u64 = 3_000_000;
    const BACKING: u64 = 1_000_000;
    ...
    .expect("provider funds the short-loss backing domain");
    ...
    .expect("honest oracle commits the adverse mark");
    ...
    let (_, live) =
        percolator_prog::state::read_market(&env.svm.get_account(&env.market).unwrap().data)
            .unwrap();
    assert_eq!(live.assets[0].effective_price, EXIT_PRICE);
    let winner_claim = live.source_credit[1].positive_claim_bound_num;
    let reserved_backing = live.source_credit[1].fresh_reserved_backing_num;
    assert!(
        winner_claim > u128::from(SHORT_CAPITAL) * percolator::BOUND_SCALE,
        "the winner claim must exceed the losing trader's principal",
    );
    ...
    if escaped {
        assert_eq!(token_amount(&env.svm, provider_destination), BACKING);
        assert!(winner_shortfall > 0);
        assert!(reserved_backing < winner_claim);
    } else {
    ...
    assert!(
        withdrawal.is_err(),
        "provider escaped {BACKING} backing atoms after the committed mark; independent winner payout={long_payout}, expected={full_winner_payout}, shortfall={winner_shortfall}, loser payout={short_payout}",
    );
}
