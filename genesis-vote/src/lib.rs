...
// claim_window[104..112], total_supply[112..120].
...
//   distribution_proposal, system, distribution_config]
fn register_proposal<'a>(program_id: &Pubkey, accounts: &'a [AccountInfo<'a>]) -> ProgramResult {
    ...
    let distribution_total_supply = {
        ...
        if *distribution_config.key != config.distribution_config
            ...
            || dc.len() < DIST_CONFIG_TOTAL_SUPPLY_OFF + 8
            || dc[..8] != DIST_CONFIG_DISC
        {
            return Err(ProgramError::InvalidAccountData);
        }
        u64::from_le_bytes(
            dc[DIST_CONFIG_TOTAL_SUPPLY_OFF..DIST_CONFIG_TOTAL_SUPPLY_OFF + 8]
                .try_into()
                .unwrap(),
        )
    };
